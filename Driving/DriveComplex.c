...
task openclaw(){				// the task to open the claw
	motor[port5] = 127;		// make the claw motor open
	wait1Msec(2000);			// wait for two seconds
	motor[port5] = 0;			// stop opening the claw so that the motor won't get stripped
}

task closeclaw(){				// the task to close the claw!
	motor[port5] = -127;	// close the claw fast at first
	wait1msec(1500);			// for 1.5 seconds, and then
	motor[port5] = -100;	// constantly close the claw to hold a cube at a lower power (no gear strip)
}
