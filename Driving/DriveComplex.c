...
task openclaw(){				// the task to open the claw
	...
	wait1Msec(500);			// wait for two seconds
...
task closeclaw(){				// the task to close the claw!
	...
	wait1Msec(500);			// for 1.5 seconds, and then
	motor[port7] = 50;	// constantly close the claw to hold a cube at a lower power (no gear strip)
}
...
task main(){
	...
	while(1){ // Do this forever, unless the task is stopped.
		...
		if(vexRT[Btn7R] && !vexRT[Btn7L]){					// if 7l is pressed and 7r is not
		...
		} else if(vexRT[Btn7L] && !vexRT[Btn7R]){ 	// otherwise, if 7r is on and 7l isn't..
